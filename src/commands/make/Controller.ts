...
export default class MakeController extends Common {
    public static async run() {
        ...
        let type = (
            await this.getListInput("What type of controller is this?", [
                "Basic",
                "Resource",
                "API",
            ])
        ).toLowerCase() as ControllerType;
        ...
        let modelToUseCommand = refModel
            ? `--model=${modelToUse} --no-interaction`
            : "";
        let typeCommand =
        ...
        this.execCmd(command, async (info) => {
            ...
            if (info.err) {
                ...
                await this.openFile(
                    info.artisan.dir,
                    "/app/Http/Controllers/" + ctrlName + ".php",
                );
            }
        });
    }
