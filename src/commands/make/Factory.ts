...
export default class MakeFactory extends Common {
    public static async run() {
        ...
        let hasModel = await this.getYesNo(
            "Is there a model related to this factory?",
        );
        let modelName = "";
        if (hasModel) {
            modelName = await this.getInput(
                "Does the model have a name? Leave blank to use (Model::class)",
            );
        }
