...
export default class Server extends Common {
    ...
    public static async run(useDefaults = false) {
        ...
        const host = useDefaults
            ? defaultHost
            : await this.getInput(
                  `Should I use a specific host (Default: ${defaultHost})?`,
              );
        const port = useDefaults
            ? defaultPort
            : await this.getInput(
                  `Should I use a specific port (Default: ${defaultPort})?`,
              );
        ...
        if (wsl) {
            ...
            artisanToUse = artisanToUse
                .replace(/^(\w):/, "/mnt/$1")
                .replace(/\\/g, "/");
        }
        setTimeout(() => {
            Server.terminal.sendText(
                `${phpLocation} "${artisanToUse}" serve --host=${Server.host} --port=${Server.port}`,
            );
            this.showMessage(
                `The server is now running on "http://${Server.host}:${Server.port}"`,
            );
        }, 1000);
    }
    ...
    public static async stop() {
        if (Server.terminal) {
            ...
            this.showMessage(
                `The server has been stopped on "http://${Server.host}:${Server.port}"`,
            );
        } else {
