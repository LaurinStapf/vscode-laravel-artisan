...
export default class RunCommand extends Common {
    ...
    static commands$ = this.commands.pipe(
        map((commands) => {
            if (
                ...
                Math.abs(Date.now() - this.lastUpdateTime) >
    ...
    public static async run() {
        ...
        this.sub = this.commands$.subscribe(async (commands) => {
            ...
            let items = commands.reduce(
                (a, v) => a.concat(v.name + " -- " + v.description),
                [],
            );
            ...
            let commandSettings = commands.find(
                (c) => c.name === cmd.split("--")[0].trim(),
            );
            ...
            for (let arg of commandSettings.arguments) {
                let input = await this.getInput(
                    `${arg.name} ${this.getDefaultText(arg)} ${this.getDescription(arg)}`,
                );
                if (this.isValidInput(input)) {
                    args.push(
                        `${input.toString().length > 0 ? input : this.toValidInput(arg.default)}`,
                    );
                }
            }
            ...
            for (let opt of commandSettings.options) {
                if (opt.accept_value) {
                    let input = await this.getInput(
                        `${opt.name} ${this.getDefaultText(opt)} ${this.getDescription(opt)}`,
                    );
                    if (this.isValidInput(input)) {
                        let val = "";
                        opts.push(
                            `${opt.name}${opt.is_value_required ? `=${input.length > 0 ? input : this.toValidInput(opt.default)}` : ""}`,
                        );
                    }
            ...
            this.execCmd(command, (info) => {
                if (info.err) {
                    ...
                    this.showMessage(
                        `Command "${commandSettings.name}" has finished ${msg}`.trim(),
                    );
                }
            });
        });
    ...
    private static getDefaultText(obj) {
        return this.isValidInput(obj.default) &&
            obj.default.toString().length > 0
            ? `[${obj.default}]`
            : "";
    }

    private static getDescription(obj) {
        return obj.description.toString().length > 0
            ? `(${obj.description})`
            : "";
    }
