...
export default class Common {
    ...
    protected static async execCmd(
        ...
        if (dockerEnabled) {
            ...
            const quotedInvoker =
                phpInvoker.includes(" ") && !phpInvoker.startsWith("herd ")
                    ? `"${phpInvoker}"`
                    : phpInvoker;
    ...
    private static herdCheckCache = new Map<
        string,
        "managed" | "unmanaged" | "unavailable"
    >();
    ...
    private static async isHerdManagedProject(cwd: string): Promise<boolean> {
        ...
        try {
            ...
            if (
                /not recognized|command not found|No such file|CommandNotFoundException/i.test(
                    out + err,
                )
            ) {
            ...
            if (
                /Herd Desktop application is not running|Command "site-information" is not defined/i.test(
                    out + err,
                )
            ) {
                this.herdCheckCache.set(cwd, "unavailable");
